...
class SendCloudMsg(object):
    def process_item(self, item, spider):
        ...
        if is_local == 1:
            print("------------Sent to Cloud!--------------")
            r1 = requests.post("https://www.tangwei.cc/api/receive_neteasetop", data=item)
            print(r1)
        return item
...
class SendWarnMsgToWeixinApp(object):
    def process_item(self, item, spider):
        ...
        if is_local_result == 1:
            ...
            baidu_top_title = "【网易热榜】" + item['title']
...
class SendItemToFindTopic(object):
    def process_item(self, item, spider):
        is_local_result = item['is_local']
        if is_local_result == 1:
            print("------------Send Netease Top To AI Parse!--------------")
            data = {
                "type": "netease",
                "title":item['title'],
                "text":item['text'],
                "add_time":item['add_time'],
                "hot_num": self.format_hot_num("netease",item['hotScore']),
                "img":item['img'],
                "url":item['url']
            }
