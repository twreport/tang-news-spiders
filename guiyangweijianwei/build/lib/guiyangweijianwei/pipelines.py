...
redis_db_title = Redis(host='10.168.1.100', port=6379, db=7)  # 连接redis
redis_data_dict_title = "f_title"  # key的名字，写什么都可以，这里的key相当于字典名称，而不是key值。


class GuiyangweijianweiPipeline:
...
class SendWeixinMsg(object):
    def process_item(self, item, spider):
        if '新冠肺炎' in item['text'] or '确诊病例' in item['text'] or '无症状感染者' in item['text'] or '疫情' in item['text'] or '静态' in item['text']:
            print("------------sendwecom!--------------")
            text = '【贵阳市卫健委官网最新发布】'
            text += '\n'
            text += item['title']
...
class DuplicatesPipeline(object):
    ...
    def __init__(self):
        ...
        if redis_db_title.hlen(redis_data_dict_title) == 0:
            sql = "SELECT title FROM tw_guiyangweijianwei ORDER BY id DESC limit 0,20;"  # 从你的MySQL里提数据，我这里取title来去重。
...
class MySQLPipeline(object):
    ...
    def process_item(self, item, spider):
        print("------------in mysql---------------")
        print(item)
        insert_sql = """insert into tw_guiyangweijianwei(title, url, 
